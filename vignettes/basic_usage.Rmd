...
```{r, eval=FALSE}
mbres <- motifbreakR(myprosnps, pwmList=motifs)
```

Additionally, let's run motifbreakR on _all_ SNPs for promoter regions in _all_ genes.

```{r, eval=FALSE}
mball <- parallel_motifbreakR(prosnps_list, pwmList=motifs)
```
...
```{r, eval=FALSE}
res <- mbres |> mb_to_tibble()
all <- mball |> mb_to_tibble()
```
...
# Save the data
# vignettedata <- list()
# vignettedata$res <- res
# vignettedata$all <- all
# usethis::use_data(vignettedata)
```
